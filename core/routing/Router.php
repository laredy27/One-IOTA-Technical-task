...
class Router {
	...
	protected function runController(Route $route, Request $request) {
		$controllerName = $route->getController();
                ...
                $args = substr($requestPath, strpos($requestPath, $methodName) );
                $methodArgs = explode(DIRECTORY_SEPARATOR, $args);
		//remove method name from $methodArgs
		...
		//return call_user_func_array(array($controller, $methodName), array());
                return call_user_func_array(array($controller, $methodName), $methodArgs);
	}
